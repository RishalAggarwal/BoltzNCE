...
class GVP_vector_field(torch.nn.Module):
    def __init__(self, n_features=21, n_layers=5, n_hidden=64,n_vec=16,n_message_gvps=1,n_update_gvps=1,n_coord_gvps=1,num_particles=22, use_dst_feats=False,vector_gating=True,self_conditioning=False):
        ...
        if self_conditioning:
            self.conditional_embedding = torch.nn.Sequential(torch.nn.Linear(n_features, n_hidden),nn.SiLU())
            self.conditional_convolution = GVPConv(scalar_size=n_hidden,vector_size=n_vec,n_message_gvps=n_message_gvps,n_update_gvps=n_update_gvps,use_dst_feats=use_dst_feats,vector_gating=vector_gating,coords_range=10,scalar_activation=SwishBeta)
    ...
    def forward(self,t,data,condition=None):
        ...
        if condition is not None and self.self_conditioning:
            ...
            v_cond=v_init.clone()
            h_cond,v_cond,x_cond=self.conditional_convolution(data,z_cond,condition,v_cond)
            zs = zs + h_cond

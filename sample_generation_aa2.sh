...
#SBATCH -x g020
#SBATCH --array=0-200          # 200 prefixes → indices 0 through 199
...
#prefixes=("DI" "RM" "CY" "MP" "GF" "SC" "EG" "HA" "AE" "YH" "WA" "TN" "LC" "MM" "IR" "QL" "WR" "YF")
...
python infer_aa2.py \
  --config configs/infer_vector_aa2_correctedbias.yaml \
  --wandb_inference_name inference_aa2_vector_correctedbias_${prefix}_50k \
  --peptide "${prefix}" \
  --save_generated \
  --save_prefix "./generated/correctedbias/" \
  --no-divergence \
